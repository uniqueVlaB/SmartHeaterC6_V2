...
WebApi::WebApi(HeaterController& ctrl, TempSensors& temp,
               ...
               Stats& stats, StatusLed& led, uint16_t port)
    : _server(port), _ctrl(ctrl), _temp(temp), _pzem(pzem), _relay(relay),
      _scheduler(scheduler), _timeSync(timeSync), _stats(stats), _led(led) {}
...
void WebApi::begin() {
    ...
    _server.on("/stats/reset",        HTTP_POST, [this]() { handleStatsReset();     });
    // ── System ──────────────────────────────────────────
